...
function Saving(actiontype){
     ...
     if(actiontype=="approve"){
		...
		if(confirm(" Total Cgpan Selected : "+totSelectCnt+" \n Total Net Outstanding Amount : "+totNetAmt+" \n\n !! Are you Sure for Approve for  Payment !! " )){
			  ...
			  submitForm('ClaimSettlePaymentSavedMLIWiseCK2Approval.do?method=ClaimSettlePaymentSavedMLIWiseCK2Approval&actionType=Approve')
              return true;
    	 ...
    	 if(confirm(" Total Cgpan Selected : "+totSelectCnt+" \n Total Net Outstanding Amount : "+totNetAmt+" \n\n !! Are you Sure for Return for Payment !! " )){
			  ...
			  submitForm('ClaimSettlePaymentSavedMLIWiseCK2Approval.do?method=ClaimSettlePaymentSavedMLIWiseCK2Approval&actionType=Reject')
              return true;
		}else{
											...
											class="ColumnBackground1">
											    ...
											    <%
											     if(value.get(i)==null){
											    	 ...
											    	 if(i==position_payid){%>
											           <a href="ShowPaymentAllocatedMLIWiseData.do?method=ShowPaymentAllocatedMLIWiseData&PayId=<%=value.get(position_payid)%>&approvalStatus=RA"><%=value.get(i)%></a>	 
                                                    <%
											    	 }else{%>
                                                       <%=value.get(i)%>											    		 
											    	 <%}
				...
				<TR>
					...
					<table cellspacing=5 cellpadding=5>
						<tr>
							...
							<td>
								<a href="ClaimSettleForPayment.do?method=ExportToFile&&fileType=CSVType&FlowLevel=CK2">
								<IMG src="images/csv_icon.png" width="50" height="35" alt="Export to Excel">
								</a>
							</td>
							
							<td>
								<input type="button" value=" Approve Payment " onClick="Saving('approve');">
							</td>
					...
					<DIV align="center">
					<A	href="javascript:submitForm('ClaimSettlePaymentSavedMLIWiseCK2Input.do?method=getClaimSettlePaymentSavedMLIWiseCK2Input')">
