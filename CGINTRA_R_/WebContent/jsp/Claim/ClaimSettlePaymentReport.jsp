...
function checkAll(ele) {
    ...
    if (ele.checked) {
            ...
            if (ElemItem[i].type == 'hidden') {
               ...
               var netTotTemp=0.0;
                if(i!=NoOFEle && i!=(NoOFEle-1) && i!=(NoOFEle-2)&&i!=(NoOFEle-3)){
                    ...
                    netTotTemp=parseFloat(ElemItem[NoOFEle-1].value)+parseFloat(ElemItem[i].value);
                    ElemItem[NoOFEle-1].value=netTotTemp.toFixed(2);
                }               
            }

			if(ElemItem[i].type == 'text'){
				ElemItem[NoOFEle-2].value=NoOFChkBox;
			}
        }
    } else {
        ...
        ElemItem[NoOFEle-1].value=netamtZero.toFixed(2);/*for amt*/
        ElemItem[NoOFEle-2].value=0;/*for cnt*/
    }
}
...
function Saving(actiontype){
     ...
     if(actiontype=="approve"){
		...
		if(confirm(" Total Cgpan Selected : "+totSelectCnt+" \n Total Net Outstanding Amount : "+totNetAmt+" \n\n !! Are you Sure for Initiate Payment !! " )){
			  ...
			  submitForm("ClaimSettlePaymentSavedMLIWiseCK1Approval.do?method=ClaimSettlePaymentSavedMLIWiseCK1Approval&actionType=Approve");
              return true;
    	 ...
    	 if(confirm(" Total Cgpan Selected : "+totSelectCnt+" \n Total Net Outstanding Amount : "+totNetAmt+" \n\n !! Are you Sure for Return Initiate Payment !! " )){
			  ...
			  submitForm("ClaimSettlePaymentSavedMLIWiseCK1Approval.do?method=ClaimSettlePaymentSavedMLIWiseCK1Approval&actionType=Reject");
              return true;
		}else{
...
<bean:define id="status" name="claimSettleForPaymentForm" property="status" type="java.lang.String"/> 
<%String var_status=status;
String status_value="";
if(var_status.equals("MA")){
	
	status_value="Maker saved for Initiated";
	
}else if(var_status.equals("MI")){
	
	status_value="Maker Initiated";
	
}else if(var_status.equals("RA")){
	
	status_value="Reviewer Approved";
	
}else if(var_status.equals("CA")){
	
	status_value="Checker Approved";
	
}else if(var_status.equals("CAPB")){
	
	status_value="Checker Approved Claim Settled Data for Payment Bank";
}
					...
					<TABLE width="100%" border="0" cellspacing="1" cellpadding="1">
						<TR>
							...
							<TABLE width="100%" border="0" cellpadding="0" cellspacing="0">
								...
								<TR>
									<TD class="Heading"><%=status_value%></TD>
									<td class="Heading" >&nbsp;
						...
						<TD width="10%" align="center" valign="top"	class="ColumnBackground1">Select All<br>
								       ...
								       <%=index1+1%>
								        ...
								        <TD width="10%" align="left" valign="top"
											...
											%>
											    ...
											    <html:checkbox name="claimSettleForPaymentForm"  property="<%=id%>" value="true" onclick="<%=jsfuncName%>"  ></html:checkbox>
									    </TD>
									<%	
									      for(int i=0;i<value.size();i++){
									%>
										...
										<!-- <%=value.get(i)==null?"--":value.get(i)%>-->
											    <%
											     if(value.get(i)==null){
											    	 ...
											    	 if((i==position_payid) && (!var_status.equals("CAPB"))){%>
											           <a href="ShowPaymentAllocatedMLIWiseData.do?method=ShowPaymentAllocatedMLIWiseData&PayId=<%=value.get(position_payid)%>&approvalStatus=<%=var_status%>"><%=value.get(i)%></a>	 
                                                    <%
											    	 }else{%>
                                                       <%=value.get(i)%>											    		 
											    	 <%}
				...
				<TR>
					...
					<table cellspacing=5 cellpadding=5>
						<tr>
							...
							<td>
								<a href="ClaimSettleForPayment.do?method=ExportToFile&&fileType=CSVType&FlowLevel=REPORT">
								<IMG src="images/csv_icon.png" width="50" height="35" alt="Export to Excel">
								</a>
							</td>
							 
							<td>
								<a href="ClaimSettleForPayment.do?method=ExportToFile&&fileType=PDFType&FlowLevel=REPORT">
					...
					<DIV align="center"><A	href="javascript:submitForm('ClaimSettlePaymentReportInput.do?method=ClaimSettlePaymentReportInput')">
