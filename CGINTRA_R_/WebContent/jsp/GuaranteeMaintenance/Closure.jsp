...
session.setAttribute("CurrentPage","showClosureDetails.do?method=showClosureDetails");%>

<TABLE width="725" border="0" cellpadding="0" cellspacing="0">
	<html:errors />
	<html:form action="saveClosureDetails.do?method=saveClosureDetails" method="POST" enctype="multipart/form-data">
		<TR> 
				...
				<TABLE width="100%" border="0" align="left" cellpadding="0" cellspacing="0">
					<TR>
						<TD>
							<TABLE width="100%" border="0" cellspacing="1" cellpadding="1">
								...
								<TR> 
									<TD colspan="4">
										...
										%>
										<TR align = "center">
										<logic:iterate id="object" name="gmPeriodicInfoForm" property="closureDetails">
										<%
							...
							href="javascript:submitForm('showBorrowerDetailsLink.do?method=showBorrowerDetailsLink&formFlag=closure&bidLink=<%=borrowerId%>')"><%=borrowerId%></A>
										  </TD>
...
<!--										  <TD class="tableData"></TD>
										  <td class="tableData"></TD>
	-->									  <TD class="tableData">&nbsp;<a href="BorrowerDetailsClosure.html"><%=borrowerId%></a>
										  </TD>
								...
								href="javascript:submitForm('showCgpanDetailsLink.do?method=showCgpanDetailsLink&cgpanDetail=<%=cgpan%>')"><%=cgpan%></A>
										...
										<%}%>
										  ...
										  <% reason = cgpanInfo.getReasonForClosure();
											  name = "reasonForClosure("+cgpan+")";%>
											  
											  <html:select property="<%=name%>" name = "gmPeriodicInfoForm" value = "<%=reason%>">
											  <html:option value="">Select</html:option>
											  <html:options property="closureReasons" name="gmPeriodicInfoForm"/>	
											  </html:select>
										  </div>
										  </TD>

										  <TD class="tableData"> <div align="center"> 
											<% name = "closureFlag("+cgpan+")";%>
											 <html:checkbox property="<%=name%>" alt="Close" name="gmPeriodicInfoForm" value="Y"/>
											</div>
										 </TD>
			  							  <TD class="tableData" valign="top" align="center">
										  <% remarks = cgpanInfo.getRemarks();
											  name = "remarksForClosure("+cgpan+")";%> <html:textarea property = "<%=name%>"  name="gmPeriodicInfoForm" 
										  value="<%=remarks%>" />
								          </TD>	
...
<html:hidden property = "closureIndex" name="gmPeriodicInfoForm" value="<%=String.valueOf(closureIndex)%>"/>
								</TABLE>
							 </TABLE>
						</TD>
					</TR>
					...
					<TR >
						<TD align="center" valign="baseline" >
							<DIV align="center">
									...
									<A href="javascript:submitForm('saveClosureDetails.do?method=saveClosureDetails')">
